...
class loo_imglist extends WP_Widget {
	function loo_imglist() {
		...
		$this->WP_Widget( 'loo_imglist', 'MUFENG-图文展示', $widget_ops, $control_ops );
	}
	function widget( $args, $instance ) {
		...
		echo ltheme_posts_list( $orderby,$limit,$cat);
		echo '</ul></div>';
		echo $after_widget;
	}
...
function ltheme_posts_list($orderby,$limit,$cat) {
	$args = array(
		'order'            => DESC,
		'cat'              => $cat,
		'orderby'          => $orderby,
...
?>
        <li class="post">
            <div class="imgss"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php echo post_thumbnail_img(142,95);?></div>
            <h4><?php the_title(); ?></h4></a>
            <div class="imgtimes">
            <span class="info_date "><?php the_time('Y-m-d')?></span>/
            <span class="info_views "><?php echo getPostViews(get_the_ID());?>人围观</span></div>
        </li>
<?php
