...
class loo_comment extends WP_Widget {
	function loo_comment(){
		$widget_ops = array('description' => 'MUFENG-最新评论');
		parent::WP_Widget('loo_comment',$name='MUFENG-最新评论',$widget_ops);
		//parent::直接使用父类中的方法
		//$name 这个小工具的名称,
...
<?php
	...
	if( function_exists('wp_recentcomments') ) :
		...
		foreach($comments as $comment){ $cmt_visitor = get_comment_author(); echo '<li><a href="'.get_comment_link($comment->comment_ID).'" title="查看该评论文章： '.get_the_title($comment->comment_post_ID).'">'.get_avatar( $comment, 32 ).'<span>'.get_comment_author($comment->comment_ID).'</span><br/>'.convert_smilies(mb_strimwidth(get_comment_text($comment->comment_ID), 0, 25, '...','utf-8')).'</a></li>';}
	endif;
	wp_reset_query();
	echo '</ul></div>';
	}
